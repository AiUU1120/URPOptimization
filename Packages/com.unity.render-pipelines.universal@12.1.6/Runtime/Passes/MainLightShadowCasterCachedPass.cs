...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class MainLightShadowCasterCachedPass : ScriptableRenderPass
    {
        ...
        private int[] m_InterleavePattern = new int[]
        {
            1 | 4,
            1 | 8,
            2 | 8,
        ...
        private int m_InterleaveIndex = 0;
        ...
        public bool Setup(ref RenderingData renderingData)
        {
            ...
            // 修改ShadowMap创建逻辑
            var mask = m_InterleavePattern[m_InterleaveIndex];
            for (var cascadeIndex = 0; cascadeIndex < m_ShadowCasterCascadesCount; ++cascadeIndex)
            {
                var current = 1 << cascadeIndex;
                Vector4 splitData;
                var success = ShadowUtils.ExtractDirectionalLightMatrix(ref renderingData.cullResults, ref renderingData.shadowData,
                    shadowLightIndex, cascadeIndex, renderTargetWidth, renderTargetHeight, shadowResolution, light.shadowNearPlane,
                    out splitData, out m_CascadeSlices[cascadeIndex]);

                if (!success)
                    return SetupForEmptyRendering(ref renderingData);
                if ((current & mask) != 0 || m_FirstFrame)
                    m_CascadeSplitDistances[cascadeIndex] = splitData;
            }

            if (m_MainLightShadowmapTexture == null || m_MainLightShadowmapTexture.width != renderTargetWidth || m_MainLightShadowmapTexture.height != renderTargetHeight)
            {
        ...
        public override void OnCameraCleanup(CommandBuffer cmd)
        {
            ...
            // 移除m_MainLightShadowmapTexture释放逻辑
            // if (m_MainLightShadowmapTexture)
            // {
            //     RenderTexture.ReleaseTemporary(m_MainLightShadowmapTexture);
            //     m_MainLightShadowmapTexture = null;
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.MainLightShadow)))
            {
                ...
                for (var cascadeIndex = 0; cascadeIndex < m_ShadowCasterCascadesCount; ++cascadeIndex)
                {
                    ...
                    if (m_FirstFrame || (current & mask) != 0)
                    {
                        ...
                        var shadowBias = ShadowUtils.GetShadowBias(ref shadowLight, shadowLightIndex, ref shadowData, m_CascadeSlices[cascadeIndex].projectionMatrix,
                ...
                //     settings.splitData = m_CascadeSlices[cascadeIndex].splitData;
                //
                //     Vector4 shadowBias = ShadowUtils.GetShadowBias(ref shadowLight, shadowLightIndex, ref shadowData, m_CascadeSlices[cascadeIndex].projectionMatrix,
                //         m_CascadeSlices[cascadeIndex].resolution);
                //     ShadowUtils.SetupShadowCasterConstantBuffer(cmd, ref shadowLight, shadowBias);
                //     CoreUtils.SetKeyword(cmd, ShaderKeywordStrings.CastingPunctualLightShadow, false);
